...
export function useSolanaCostCalculator() {
  ...
  const calculateCosts = useCallback(({ numAccounts, dataSizePerAccountBytes }: CostParams) => {
    if (numAccounts <= 0 || dataSizePerAccountBytes < 0) {
      setResults(null);
      return;
    }
    ...
    const uncompressedCostSol = totalBytesUncompressed * COST_PER_BYTE_PER_YEAR_SOL;
    const uncompressedCostUsd = uncompressedCostSol * SOL_PRICE_USD;

    // Compressed cost calculation (simplified)
    // Assuming a very small on-chain footprint + amortized proof costs.
    // For ZK compressed accounts, the "dataSizePerAccountBytes" is largely off-chain.
    // The on-chain cost is more about the state commitment in the tree.
    const compressedCostSol = numAccounts * ZK_COMPRESSED_ACCOUNT_FLAT_COST_SOL_PER_YEAR;
    ...
    setResults({
      ...
      compressedCostSol,
      compressedCostUsd,
      savingsSol: Math.max(0, savingsSol), // Ensure savings are not negative
      savingsUsd: Math.max(0, savingsUsd),
      savingsPercentage: Math.max(0, Math.min(100, savingsPercentage)), // Clamp percentage
    });
  }, []);
